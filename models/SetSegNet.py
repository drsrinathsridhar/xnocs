...
class SetSegNet(ptNets.ptNet):
    def __init__(self, n_classes=21, in_channels=3, is_unpooling=True, Args=None, DataParallelDevs=None, pretrained=True, withSkipConnections=False, enablePermEq=True):
        super().__init__(Args)
        ...
        if pretrained:
            ...
            if self.withSkipConnections:
                Arch = 'SetSegNetSkip'
            print('[ INFO ]: Using pre-trained weights from VGG16 with {}. Permutation equivariant layers are {}.'.format(Arch, 'ENABLED' if self.enablePermEq else 'DISABLED'))
    ...
    def forward(self, inputs):
        ...
        if self.withSkipConnections:
            ...
            ups1 = []
            for s in range(S):
                up5 = self.up5(torch.squeeze(downs5[:, s, :, :, :], dim=1), all_indices_5[s], all_unpool_shape5[s], SkipFeatureMap=all_FM5[s])
            ...
            for s in range(S):
                up4 = self.up4(torch.squeeze(ups5[:, s, :, :, :], dim=1), all_indices_4[s], all_unpool_shape4[s], SkipFeatureMap=all_FM4[s])
            ...
            for s in range(S):
                up3 = self.up3(torch.squeeze(ups4[:, s, :, :, :], dim=1), all_indices_3[s], all_unpool_shape3[s], SkipFeatureMap=all_FM3[s])
            ...
            for s in range(S):
                up2 = self.up2(torch.squeeze(ups3[:, s, :, :, :], dim=1), all_indices_2[s], all_unpool_shape2[s], SkipFeatureMap=all_FM2[s])
            ...
            for s in range(S):
                up1 = self.up1(torch.squeeze(ups2[:, s, :, :, :], dim=1), all_indices_1[s], all_unpool_shape1[s], SkipFeatureMap=all_FM1[s])
