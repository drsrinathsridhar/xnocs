...
Modes = ['train']
Datasets = ['MShapeNetCOCODataset']
BatchSizes = [1, 2] #1, 2, 4, 8] # LABEL
LearningRates = [0.0001] #[0.1, 0.01, 0.001, 0.0001]#[, 0.00005, 0.00001]  # LABEL
Archs = [ 'SetSegNetSkipPermEq' ]# ['SegNet', 'SegNetSkip']  # LABEL
...
SetSizes = [5] #[2, 3, 5, 10, 12]
VariableSetSize = ['--enable-variable-set-size'] #['', '--enable-variable-set-size']

# Problem-specific config
NOCSConfigList = ['--mode={}', '--gpu={}', '--seed={}', '--data-limit={}', '--load-memory'
    , '--dataset={}', '--arch={}', '--val-data-limit={}', '--category={}', '{}', '{}', '--loss={}'
                  , '--set-size={}', '{}']
# Network config
...
if __name__ == '__main__':
    ...
    GridSearchParams = list(itertools.product(Modes, BatchSizes, LearningRates, Datasets, Archs, Categories, MaskStatus, HallucinateStatus, Losses, SetSizes, VariableSetSize))
    ...
    for Ctr, Config in enumerate(GridSearchParams, 0):
        ...
        NewNOCSConfig[12] = NewNOCSConfig[12].replace('{}', str(Config[9]))  # Set size
        NewNOCSConfig[13] = NewNOCSConfig[13].replace('{}', str(Config[10]))  # Variable set size

        NOCSConfig = mv_nxm_config.mv_nxm_config(NewNOCSConfig)
        ...
        if Config[6] is not '--no-mask':
            ExptName += '_WithMask'
        ExptName += '-SetSize_' + str(Config[9]).zfill(2)
        if Config[10] == '--enable-variable-set-size':
            ExptName += '_Var'
        else:
            ExptName += '_Fixed'
