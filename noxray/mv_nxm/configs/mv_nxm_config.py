...
class mv_nxm_config():
    def __init__(self, InputArgs=None):
        self.Parser = argparse.ArgumentParser(description='NOXRay maps prediction for multiple views.', fromfile_prefix_chars='@')
        ...
        # Experiment control
        ArgGroup.add_argument('--dataset', help='Choose dataset.', choices=['MShapeNetCOCODataset'], default='MShapeNetCOCODataset')
        ...
        ArgGroup.add_argument('--arch', help='Choose architecture.', choices=['SetSegNet', 'SetSegNetSkip', 'SetSegNetSkipPermEq'], default='SetSegNetSkip')
        ...
        ArgGroup.add_argument('--enable-variable-set-size', help='Enable variable number of views in a batch.', action='store_true')
        self.Parser.set_defaults(enable_variable_set_size=False)
        ArgGroup.add_argument('--set-size', help='Choose (max) number of views to train on.', type=int, default=-1, choices=range(-1,50))
        ...
        # Loss function
        ArgGroup.add_argument('--loss', help='When using mask, what loss to use?', choices=['l2'], default='l2')
